...
class BeatLoopRollButton extends TriggerButton {
	constructor(options) {
		...
		if (BeatLoopRolls[options.number] === 'half') {
			...
			if (isNaN(size)) {
				throw Error(
					`BeatLoopRollButton ${options.number}'s size "${BeatLoopRolls[options.number]}" is invalid. Must be a float, or the literal 'half' or 'double'`
				);
			}
...
class Mixer extends ComponentContainer {
	constructor(inReports, outReports) {
		...
		this.mixerColumnDeck1 = new S4Mk3MixerColumn(
			1,
			inReports,
			outReports[128],
			{
		...
		this.mixerColumnDeck2 = new S4Mk3MixerColumn(
			2,
			inReports,
			outReports[128],
			{
		...
		this.mixerColumnDeck3 = new S4Mk3MixerColumn(
			3,
			inReports,
			outReports[128],
			{
		...
		this.mixerColumnDeck4 = new S4Mk3MixerColumn(
			4,
			inReports,
			outReports[128],
			{
...
class S4Mk3Deck extends Deck {
	constructor(
		...
		this.turntableButton = UseMotors
			? new Button({
					deck: this,
					...
					outTrigger: function () {
						...
						this.deck.jogButton.send(
							this.color +
								(vinylModeOn ? this.brightnessOn : this.brightnessOff)
						);
					},
				})
			: undefined;
...
class S4MK3 {
	constructor() {
		...
		this.effectUnit1 = new S4Mk3EffectUnit(
			1,
			this.inReports,
			this.outReports[128],
			{
		...
		this.effectUnit2 = new S4Mk3EffectUnit(
			2,
			this.inReports,
			this.outReports[128],
			{
